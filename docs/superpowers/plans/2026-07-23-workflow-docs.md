# Workflow Documentation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a "Recommended Workflow" guide (`docs/workflow.md`) and surface the plugin harness in `README.md`.

**Architecture:** Documentation-only change driven by
`docs/superpowers/specs/2026-07-23-workflow-docs-design.md`. Two units:
a new narrative guide (`docs/workflow.md`) walking the six-stage feature
cycle, and three surgical README edits (opening paragraph, new `## Workflow`
section with a Mermaid diagram, Documentation link). No plugin is modified.

**Tech Stack:** GitHub-flavored Markdown, Mermaid (rendered natively by GitHub).

## Global Constraints

- All committed content is in **English**.
- **No changes to any plugin** — no version bumps, no `marketplace.json`
  changes, no `docs/plugins/*.md` edits.
- README plugin-count badge (`plugins-10-green`) and the Available Plugins
  table stay untouched.
- `workflow.md` must not reproduce flag tables, option lists, or edge-case
  descriptions from `docs/plugins/*.md` — per-stage text is limited to
  purpose, command names, artifact paths, one-line behavioral summaries,
  and cross-stage hand-offs.
- This repo blocks direct `git commit`; every commit command must be
  prefixed with `AV_COMMIT_SKILL=1`. Never push. No AI co-authorship
  trailers in commit messages.
- Work happens on the existing branch `docs/workflow-guide`.
...
**Interfaces:**
- Consumes: nothing from other tasks (relative links target existing
  `docs/installation.md` and `docs/plugins/`).
- Produces: the file `docs/workflow.md` at repo root's `docs/` — Task 2's
  README links point at `docs/workflow.md` and break if this path changes.

- [ ] **Step 1: Write the file**

Create `docs/workflow.md` with exactly this content:
...
```bash
grep -c '^### Stage' docs/workflow.md
```

Expected: `6`

```bash
grep -c '```mermaid' docs/workflow.md
```

Expected: `1`

```bash
test -f docs/installation.md && test -d docs/plugins && echo links-ok
```
...
AV_COMMIT_SKILL=1 git commit -m "docs(workflow): add the recommended workflow guide"
```

---

### Task 2: Surface the workflow in `README.md`

**Files:**
- Modify: `README.md:6` (opening paragraph), `README.md:14-16` (insert
  Workflow section between Installation and Available Plugins),
  `README.md:31-35` (Documentation list)

**Interfaces:**
- Consumes: `docs/workflow.md` from Task 1 (both links break if it is
  missing or moves).
...
- [ ] **Step 3: Add the Documentation link**
...
```bash
grep -c 'docs/workflow.md' README.md
```

Expected: `2` (Workflow section + Documentation list)

```bash
grep -c 'plugins-10-green' README.md
```

Expected: `1` (badge untouched)

```bash
grep -n '## Workflow' README.md && grep -n '## Available Plugins' README.md
```

Expected: the `## Workflow` line number is smaller than the
`## Available Plugins` line number.

- [ ] **Step 5: Commit**

```bash
git add README.md
AV_COMMIT_SKILL=1 git commit -m "docs(readme): surface the workflow harness in the README"
```
