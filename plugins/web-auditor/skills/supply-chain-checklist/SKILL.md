...
```javascript
// Playwright: get all script sources and check for sourceMappingURL
const scripts = Array.from(document.querySelectorAll('script[src]')).map(s => s.src);
// For each, the agent should fetch the JS file and check for:
