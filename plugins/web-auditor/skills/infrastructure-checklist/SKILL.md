...
What to check:
- SPF present and restrictive (ends with `-all` not `~all` or `+all`)
- DMARC present with `p=reject` or `p=quarantine` (not `p=none` in production)
- DKIM configured
- DNSSEC enabled
...
```bash
# Use the WebFetch tool to analyze https://securityheaders.com/?q=TARGET&followRedirects=on
# Extract the overall grade and per-header analysis
```
